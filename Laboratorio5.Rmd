...
texto <- VCorpus(DirSource("C:/Users/smayr/Documents/Tercer año/Semestre 6/Data Science/Laboratorio 5"), readerControl = list(language = "en"))
...
Obteniendo las palabras más frecuentes, se tomó la decisión de encontrar las que se encuentren por lo menos más de 1000 veces.
```{r}
#Se conviernten en un term document matrix
blogM <-  TermDocumentMatrix(blog)


findFreqTerms(blogM, lowfreq=1000)
...
findFreqTerms(twitterM, lowfreq=1000)
```
...
findFreqTerms(newsM, lowfreq=100)
```
...
wordcloud(twitter, max.words=50, random.order= FALSE, rot.per=0.35, 
          colors=brewer.pal(8, "Dark2"))
```
Para el archivo de Twitter, se destaca nuevamente la palabra 'will' y 'come', pero también destaca 'thank' y 'love' ¿Se podría asociar estas palabras a sentimientos positivos?
